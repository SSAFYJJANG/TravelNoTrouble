...
const filter = (theme) => {
    ...
    FILTER_SPOTS_THEME(theme);
};

const { spots, filters } = mapState("spotStore", ["spots", "filters"]);
const { FILTER_SPOTS_THEME } = mapMutations("spotStore", ["FILTER_SPOTS_THEME"]);

</script>

<template>
    <!-- <section id="recent-blog-posts" class="recent-blog-posts portfolio pt-3 pb-5"
        style="min-height: 100%; background-color: red;">
